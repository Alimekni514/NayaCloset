...
const normalizeForMatching = (value) => value
    .normalize('NFD')
    .replace(DIACRITICS_REGEX, '')
    .toLowerCase()
    .replace(/[^a-z0-9]+/gu, ' ')
    .trim();
const includesOneOf = (value, patterns) => patterns.some((pattern) => value.includes(pattern));
export const mapAbmStatusCategory = (label) => {
    const normalized = normalizeForMatching(label ?? '');
    if (!normalized) {
        return 'neutral';
    }
    if (includesOneOf(normalized, [
        'colis livre',
        'livre',
        'livree',
        'livraison effectuee',
...
export const mapAbmProgressStage = (label, category) => {
    const normalized = normalizeForMatching(label ?? '');
