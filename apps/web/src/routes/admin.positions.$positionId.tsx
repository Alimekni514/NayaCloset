...
import type { AbmPositionDetail, AbmPositionListItem } from '@delivery-commerce/shared';

import { ConfirmDialog } from '@/components/common/ConfirmDialog';
import { apiErrorUtils } from '@/lib/api-client';
import {
  fetchAbmPositionLabelDocument,
  PositionDetailError,
...
function mapDetailError(error: unknown): string {
  if (apiErrorUtils.isApiError(error)) {
    if (error.status === 400) return 'Identifiant de position invalide.';
    if (error.status === 401) return 'Votre session a expire. Veuillez vous reconnecter.';
    if (error.status === 403) return "Vous n'etes pas autorise a consulter cette position.";
    if (error.status === 404) return 'Position introuvable.';
    ...
    if (code === 'ABM_LOGIN_FAILED') return 'Impossible de se connecter a ABM.';
...
function mapDeleteError(error: unknown): string {
  if (apiErrorUtils.isApiError(error)) {
    ...
    if (code === 'ABM_POSITION_DELETE_NOT_ALLOWED') return 'Cette position ne peut plus etre supprimee.';
    if (code === 'ABM_POSITION_DELETE_FAILED') return 'Impossible de supprimer la position.';
    if (code === 'ABM_BAD_RESPONSE') return 'ABM a retourne une reponse inattendue.';
  }
...
function mapPrintError(error: unknown): string {
  if (apiErrorUtils.isApiError(error)) {
    if (error.status === 401) return 'Votre session a expire. Veuillez vous reconnecter.';
    if (error.status === 403) return "Vous n'etes pas autorise a imprimer cette etiquette.";
    if (error.status === 404) return 'Cette position est introuvable.';
    if (error.status === 502) return "Impossible de preparer l'etiquette.";
...
function mergeDetailWithListContext(
  detail: AbmPositionDetail,
  listItem: AbmPositionListItem | undefined,
): AbmPositionDetail {
  if (!listItem) {
    return detail;
  }
...
function PositionDetailPage() {
  ...
  const [printLoadingAction, setPrintLoadingAction] = useState<PositionLabelAction | null>(null);
  ...
  const position = useMemo(
    () => (detailQuery.data ? mergeDetailWithListContext(detailQuery.data, listItem) : undefined),
    [detailQuery.data, listItem],
  );
  ...
  if (!isValidPositionId(positionId)) {
    return <PositionDetailError message="Identifiant de position invalide." backSearch={backSearch} />;
  }
  ...
  if (detailQuery.isError || !position) {
    return (
      <PositionDetailError
        message={mapDetailError(detailQuery.error)}
  ...
  return (
    <div className="space-y-6">
      <PositionDetailHeader
        ...
        printLoadingAction={printLoadingAction}
        onPrint={(action) => void handlePrint(action)}
      />

      <PositionDetailSummary position={position} />

      <div className="grid gap-6 xl:grid-cols-[minmax(0,0.95fr)_minmax(0,1.55fr)]">
        <PositionTrackingTimeline events={position.events} />

        <div className="space-y-6">
          <PositionProgress position={position} />
          <PositionRouteCard position={position} />
          <PositionRecipientCard position={position} />
          <PositionShipmentCard position={position} />
          <PositionDimensionsCard position={position} />
          <PositionMetadataCard position={position} />
        </div>
      </div>
      ...
      <ConfirmDialog
        ...
        description={`Cette action ne pourra pas etre annulee. La position ${position.barcode} sera definitivement supprimee.`}
